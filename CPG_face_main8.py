...
def main():
    ...
    val_loader = val_loader(args.input_img)
        # # manager = Manager(args, model, shared_layer_info, masks, train_loader, val_loader, begin_prune_step, end_prune_step)
        # distance_metric = True
        # subtract_mean = False
    inference_dataset_idx = model.module.datasets.index(args.dataset) + 1
    pruner = SparsePruner(model, masks, args, begin_prune_step, end_prune_step,
                              inference_dataset_idx)
    pruner.apply_mask()
    model.eval()  # switch to evaluate mode
        # embedding_list_a, embedding_list_b = [], []
    with torch.no_grad():
        _, (data_a, data_p, _) = enumerate(val_loader)
        data_a, data_p = data_a.cuda(), data_p.cuda()
        data_a, data_p = Variable(data_a, volatile=True), \
                                        Variable(data_p, volatile=True)
                    # ==== compute output ====
        out_a = model.module.forward_to_embeddings(data_a)
        out_p = model.module.forward_to_embeddings(data_p)
    ...
    print(out_p)
                    # ========================
        #
        # print(embedding_list_a)
        # print(embedding_list_b)
